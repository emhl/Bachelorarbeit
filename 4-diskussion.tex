...
\end{figure}
\todo{Bezeichnung für die zwei Niveaus/Zustände?}
...
Um die beobachteten Schaltvorgänge zu charakterisieren wird der Algorithmus \todo{referenz} angewandt, welcher das Telegrafenrauschen aus dem Signal extrahiert.
...
Theoretisch müsste auch noch ein dritter, weniger stabiler, Zustand existieren, der genau zwischen den zwei Niveaus liegt. Dieser sorgt für die Abweichung von einer Gaußverteilung im Bereich zwischen den zwei Niveaus. 
...
Die kleinen Abweichungen lassen sich gut durch Probleme bei der Extraktion des Telegrafenrauschens bei einzenen Simulationsläufen (z.B. der 4. run bei \qnt{301}{\kelvin})
...
\todo{wie misst man das experimentell überhaupt?}
...
Mit der Temperatuerhöhung verschiebt sich der  erste charakteristische Peak \todo{hat der peak einen Namen?}( in \cref{fig:temp-spd-source}) nach rechts. 
...
\begin{figure}[H]
    ...
    \caption{Rauschamplitude $A$ für verschiedene Temperaturen $T$ \todo{Datenpunkte für T > 303K}}\label{fig:temp-autocorr-amplitude}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Kritische Temperatur bei schwachem und starkem Magnetfeld in c-Richtung \todo{bei 100mT noch plotten?}}\label{fig:bc-crit-temp}
\end{figure}

Bei einem starken Magnetfeld von \qnt{1}{\tesla} weicht der Reorientierungsübergang auf und es kommt kein Telegrafenrauschen mehr zu stande bei einnem schwächeren Magnetfeld (um die \qnt{30}{\milli\tesla}) verändert sich die Position des Reorientierungsübergangs nicht signifikant. Daher wurden alle Simulationen mit angelegtem Magnetfeld bei der gleichen Temperatur durchgeführt.
...
Aus mehreren verketteten Simulationsläufen ist es möglich auch für höhere Feldstärken als in \cref{fig:bc-up-percentage-violin} die Aufenthaltswahrscheinlichkeit zu bestimmen. Diese folgt einer Sigmoid-Kurve \todo{muss erklärt werden?} und oberhalb von \qnt{100}{\milli\tesla} findet quasi kein Telegrafenrauschen mehr statt.
...
Die mittlere Aufenthaltszeit ist aus dem gleichen Grund auch nichtmehr aus der Autokovarianz bestimmbar. Wann der exponentielle Abfall (Abbild \todo{wie lautet korrekter Fachbegriff?} des Telegrafenrauschen in der Autokorrelation) nichtmehr mit den anderen Charakteristika \todo{haben die einen Namen?} überlagert ist lässt sich auch direkt aus der Autokorrelation des rohen Signals erkennen:
